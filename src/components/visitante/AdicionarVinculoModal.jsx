...
import { Loader2, Plus, Info, X, FileText, AlertCircle, RefreshCcw, CheckCircle2, ArrowLeft, Clock } from 'lucide-react';
...
const AdicionarVinculoModal = ({ isOpen, onClose, user, profile, onSucesso }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length + selectedFiles.length > 8) {
      toast({
        ...
        description: "VocÃª pode enviar no mÃ¡ximo 8 arquivos por solicitaÃ§Ã£o.",
    ...
    if (invalidSize.length > 0) {
      toast({
        ...
        description: "Cada arquivo deve ter no mÃ¡ximo 5MB.",
        className: "bg-red-500 text-white border-none"
      });
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!parentesco) {
      toast({ title: "Campo obrigatÃ³rio", description: "Selecione o parentesco.", className: "bg-red-500 text-white border-none" });
      return;
    }
    if (!nomePreso.trim()) {
      toast({ title: "Campo obrigatÃ³rio", description: "Informe o nome do interno.", className: "bg-red-500 text-white border-none" });
      return;
    }
    if (matriculaObrigatoria && (!matriculaPreso || matriculaPreso.length < 6)) {
      toast({ title: "MatrÃ­cula obrigatÃ³ria", description: "Informe os 6 dÃ­gitos da matrÃ­cula para este tipo de solicitaÃ§Ã£o.", className: "bg-red-500 text-white border-none" });
      return;
    }
    if (files.length === 0) {
      toast({ title: "Documentos obrigatÃ³rios", description: "Anexe pelo menos 1 documento comprobatÃ³rio.", className: "bg-red-500 text-white border-none" });
      return;
    }
    ...
    try {
      ...
      const uploadPromise = (async () => {
        ...
        const dados = {
          ...
          cpf: profile.cpf,
          parentesco,
          nome_apenado: nomePreso.toUpperCase(),
      ...
      <DialogContent className="sm:max-w-[540px] max-h-[90vh] overflow-y-auto bg-white rounded-2xl border-0 shadow-2xl p-0">
        <DialogHeader className="p-6 pb-4 border-b border-slate-100">
          <DialogTitle className="text-[#2D5016] uppercase tracking-wider flex items-center gap-2 text-lg">
            <Plus className="w-5 h-5" /> Acrescentar VÃ­nculo
          </DialogTitle>
          <DialogDescription className="text-xs uppercase font-medium text-slate-500">
            Vincule um novo detento Ã  sua carteirinha ativa.
          </DialogDescription>
        </DialogHeader>

        <div className="p-6 pt-4">
          {/* ===== ETAPA 1: SeleÃ§Ã£o do Fluxo ===== */}
          {!fluxo ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <div className="text-center">
                    <h4 className="font-bold text-slate-900 text-sm">1Âª Via (Nova)</h4>
                    <p className="text-[10px] text-slate-500 mt-1 leading-tight">Nunca tive vÃ­nculo com este interno</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-center">
                    <h4 className="font-bold text-slate-900 text-sm">RenovaÃ§Ã£o</h4>
                    <p className="text-[10px] text-slate-500 mt-1 leading-tight">Meu vÃ­nculo anterior jÃ¡ venceu</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-center">
                    <h4 className="font-bold text-slate-900 text-sm">JÃ¡ Tenho</h4>
                    <p className="text-[10px] text-slate-500 mt-1 leading-tight">Tenho vÃ­nculo ativo e quero sincronizar</p>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-5 animate-in fade-in-50 slide-in-from-right-4 duration-300">
              {/* Header com botÃ£o Alterar */}
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2 text-sm font-bold text-slate-700">
                  {fluxo === FLUXOS.NOVA && <><FileText className="w-4 h-4 text-green-600" /> 1Âª Via (Nova)</>}
                  {fluxo === FLUXOS.RENOVACAO && <><RefreshCcw className="w-4 h-4 text-green-600" /> RenovaÃ§Ã£o</>}
                  {fluxo === FLUXOS.JA_TENHO && <><CheckCircle2 className="w-4 h-4 text-green-600" /> JÃ¡ Tenho</>}
                </div>
                <button 
                  type="button" 
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-bold uppercase text-slate-400">
                    MatrÃ­cula (6 dÃ­gitos) {matriculaObrigatoria ? '*' : '(Opcional)'}
                  </Label>
                  <Input 
                    placeholder="000000" 
                  ...
                  {!matriculaObrigatoria && (
                    <p className="text-[10px] text-blue-600 font-medium">
                      Caso nÃ£o saiba, pode deixar em branco. SerÃ¡ preenchido depois.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-red-50 border border-red-100 p-3 rounded-xl flex gap-3">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-red-800 leading-relaxed font-medium">
                    O Nome e MatrÃ­cula devem estar corretos para nÃ£o recusarem seu pedido. Consulte na <strong>Carteirinha Oficial do i-Pen</strong> ou entre em contato com o <strong>Setor Social</strong>.
                  </p>
                </div>
              </div>
              ...
              {selectedRule && (
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl space-y-1.5">
                  <div className="flex items-center gap-2 text-blue-700 font-bold text-[10px] uppercase">
                    <Info className="w-3.5 h-3.5" /> Documentos NecessÃ¡rios para "{parentesco}":
                  </div>
              ...
              <div className="space-y-3">
                <Label className="text-[10px] font-bold uppercase text-slate-400">Anexar Documentos (MÃ¡x 8 arquivos, 5MB cada) *</Label>
                <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 hover:bg-slate-100 hover:border-[#2D5016] cursor-pointer transition-all group">
                  <div className="flex flex-col items-center justify-center py-4">
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">PDF, JPG ou PNG atÃ© 5MB</p>
                  </div>
                  <input 
                    type="file" 
                    className="hidden" 
